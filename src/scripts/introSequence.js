define(function(require) {
	...
	function generatePoints() {
		...
		for (let i = 1.5 * Math.PI; i >= -0.5 * Math.PI; i -= Math.PI / 4) {
	...
	let previousAnimationStage = animationStage;
	...
	function draw() {
		...
		if (audio.currentTime >= 2.2 && audio.currentTime <= 3.1) {
			...
			animationStage = utils.map(audio.currentTime, 2.2, 3.3, 0.05, 1);
		}
